 ...
 * Endpoint: DELETE /_API/Diagnosis/DeleteDiagnosis.php?diagnosisId={id}
 * Header: token, account_token
 * 
 * Menghapus data diagnosis dari database (hard delete).
...
$Limiter->check("delete_diagnosis", 5, 60);
...
try {
    ...
    // Permission
    $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_diagnosis' LIMIT 1");
    $stmt->execute();
    $feature = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
        http_response_code(403);
        echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus diagnosis", "code" => 403], "metadata" => []]);
        exit;
    }
} catch (PDOException $e) {
    error_log('[DeleteDiagnosis] Auth error: ' . $e->getMessage());
...
try {
    $stmt = $Conn->prepare("SELECT diagnosisId FROM diagnosis WHERE diagnosisId = :id LIMIT 1");
    ...
    error_log('[DeleteDiagnosis] Check data error: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
    exit;
}
...
try {
    $stmt = $Conn->prepare("DELETE FROM diagnosis WHERE diagnosisId = :id");
    $stmt->execute([':id' => $diagnosisId]);
    ...
    error_log('[DeleteDiagnosis] Delete error: ' . $e->getMessage());
    http_response_code(500);

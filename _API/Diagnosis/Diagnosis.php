<?php
    /**
     * List Diagnosis
     * Endpoint: GET /_API/Diagnosis/Diagnosis.php?limit=10&page=1&order_by=diagnosisId&short_by=DESC&keyword_by=&keyword=
     * Header: token, account_token
     *
     * Menampilkan daftar diagnosis pasien dengan paginasi, sorting, dan pencarian.
     */
    ...
    $Limiter->check("list_diagnosis", 10, 60);
    ...
    try {
        ...
        // Validasi Permission (fitur list_diagnosis)
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = :feature_name LIMIT 1");
        $stmt->execute([':feature_name' => 'list_diagnosis']);
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Fitur list_diagnosis tidak ditemukan", "code" => 403], "metadata" => []]);
            exit;
        }
        $id_service_feature = (int) $feature['id_service_feature'];
        if (!ValidatePermission($Conn, $loggedInAccountId, $id_service_feature)) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk melihat daftar diagnosis", "code" => 403], "metadata" => []]);
            exit;
        }

    } catch (PDOException $e) {
        error_log('[ListDiagnosis] DB/Permission error: ' . $e->getMessage());
    ...
    $order_by   = isset($_GET['order_by']) ? trim($_GET['order_by']) : 'diagnosisId';
    ...
    // Set default order_by/short_by jika kosong
    if (empty($order_by)) {
        $order_by = 'diagnosisId';
    }
    if (empty($short_by)) {
        $short_by = 'DESC';
    }

    // --- 9. Daftar kolom yang diizinkan untuk sorting dan filter (hanya dari tabel diagnosis) ---
    $allowedColumns = [
        'diagnosisId', 'encounterId', 'patientId', 'idCondition',
        'medicalPersonelId', 'category', 'icdVersion', 'icdCode',
        'icdDescription', 'caseStatus', 'certaintyStatus',
    ...
    if ($keyword !== '' && $keyword_by !== '') {
        $where .= " AND d.`{$keyword_by}` LIKE :keyword";
        $params[':keyword'] = "%{$keyword}%";
    }

    // --- 14. Count total data ---
    try {
        $countSql = "SELECT COUNT(*) AS total FROM diagnosis d " . $where;
        ...
        error_log('[ListDiagnosis] Count error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    // --- 15. Query utama dengan JOIN ke patient, encounter, medical_personel, account ---
    try {
        $sql = "SELECT
                    d.diagnosisId,
                    d.encounterId,
                ...
                LEFT JOIN account cAccount ON d.creatBy = cAccount.accountId
                LEFT JOIN account uAccount ON d.updateBy = uAccount.accountId
                " . $where;
        $sql .= " ORDER BY d.`{$order_by}` {$short_by} LIMIT {$offset}, {$limit}";
        ...
        error_log('[ListDiagnosis] Query error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }

<?php
    /**
     * Delete Care Plan
     * Endpoint: DELETE /_API/CarePlan/DeleteCarePlan.php?carePlanId={id}
     * Header: token, account_token
     *
     * Menghapus data Care Plan secara hard delete.
    ...
    $Limiter->check("delete_care_plan", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_care_plan' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus Care Plan", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeleteCarePlan] Auth error: ' . $e->getMessage());
    ...
    try {
        $stmt = $Conn->prepare("SELECT carePlanId FROM care_plan WHERE carePlanId = :id LIMIT 1");
        ...
        error_log('[DeleteCarePlan] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM care_plan WHERE carePlanId = :id");
        $stmt->execute([':id' => $carePlanId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Care Plan berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [
        ...
        error_log('[DeleteCarePlan] Delete error: ' . $e->getMessage());
        http_response_code(500);

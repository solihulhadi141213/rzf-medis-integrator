<?php
    /**
     * List Care Plan
     * Endpoint: GET /_API/CarePlan/CarePlan.php?limit=10&page=1&order_by=carePlanId&short_by=DESC&keyword_by=&keyword=
     * Header: token, account_token
     *
     * Menampilkan daftar Care Plan dengan paginasi, sorting, dan pencarian.
     * Menampilkan informasi pasien, kunjungan, tenaga medis, dan pembuat/update.
     */
    ...
    $Limiter->check("list_care_plan", 10, 60);
    ...
    try {
        ...
        // Validasi Permission (fitur list_care_plan)
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = :feature_name LIMIT 1");
        $stmt->execute([':feature_name' => 'list_care_plan']);
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Fitur list_care_plan tidak ditemukan", "code" => 403], "metadata" => []]);
            exit;
        }
        $id_service_feature = (int) $feature['id_service_feature'];
        if (!ValidatePermission($Conn, $loggedInAccountId, $id_service_feature)) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk melihat daftar Care Plan", "code" => 403], "metadata" => []]);
            exit;
        }

    } catch (PDOException $e) {
        error_log('[ListCarePlan] DB/Permission error: ' . $e->getMessage());
    ...
    $order_by   = isset($_GET['order_by']) ? trim($_GET['order_by']) : 'carePlanId';
    ...
    $allowedColumns = [
        'carePlanId', 'patientId', 'encounterId', 'medicalPersonelId', 'satuSehatCode',
        'carePlanTitle', 'carePlanStatus', 'carePlanIntent',
        'carePlanCategoryName', 'carePlanCategoryCode', 'carePlanCategoryDisplay', 'carePlanCategorySystem',
        'carePlaneDescription', 'creatAt', 'updateAt', 'creatBy', 'updateBy'
    ];
    ...
    if ($keyword !== '' && $keyword_by !== '') {
        $where .= " AND cp.`{$keyword_by}` LIKE :keyword";
        $params[':keyword'] = "%{$keyword}%";
    }

    // --- 14. Count total data ---
    try {
        $countSql = "SELECT COUNT(*) AS total FROM care_plan cp " . $where;
        ...
        error_log('[ListCarePlan] Count error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $sql = "SELECT
                ...
                " . $where;
        $sql .= " ORDER BY cp.`{$order_by}` {$short_by} LIMIT {$offset}, {$limit}";
        ...
        foreach ($rows as &$row) {
            $row['carePlanId'] = (int) $row['carePlanId'];
        ...
        echo json_encode([
            "response" => [
                "message" => "Daftar Care Plan berhasil diambil",
                "code" => 200
            ],
            "metadata" => [
                "total" => $total,
        ...
        error_log('[ListCarePlan] Query error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }

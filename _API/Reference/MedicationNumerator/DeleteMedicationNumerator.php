<?php
    /**
     * Delete Medication Numerator Reference
     * Endpoint: DELETE /_API/Reference/MedicationNumerator/DeleteMedicationNumerator.php?medicationNumeratorId={id}
     * Header: token, account_token
     *
     * Menghapus data referensi numerator (medication_numerator) secara hard delete.
     */
    ...
    $Limiter->check("delete_medication_numerator", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_medication_numerator' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus referensi numerator", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeleteMedicationNumerator] Auth error: ' . $e->getMessage());
    ...
    try {
        $stmt = $Conn->prepare("SELECT medicationNumeratorId FROM medication_numerator WHERE medicationNumeratorId = :id LIMIT 1");
        ...
        error_log('[DeleteMedicationNumerator] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM medication_numerator WHERE medicationNumeratorId = :id");
        $stmt->execute([':id' => $medicationNumeratorId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Referensi numerator berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [
        ...
        error_log('[DeleteMedicationNumerator] Delete error: ' . $e->getMessage());
        http_response_code(500);

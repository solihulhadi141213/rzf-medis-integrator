<?php
    /**
     * List Medication Form Reference
     * Endpoint: GET /_API/Reference/MedicationForm/MedicationForm.php?limit=10&page=1&order_by=medicationFormId&short_by=DESC&keyword_by=&keyword=
     * Header: token, account_token
     *
     * Menampilkan daftar referensi sediaan obat (medication_form_reference) dengan paginasi, sorting, dan pencarian.
     */
    ...
    $Limiter->check("list_medication_form", 10, 60);
    ...
    try {
        ...
        // Validasi Permission (fitur list_medication_form)
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = :feature_name LIMIT 1");
        $stmt->execute([':feature_name' => 'list_medication_form']);
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Fitur list_medication_form tidak ditemukan", "code" => 403], "metadata" => []]);
            exit;
        }
        $id_service_feature = (int) $feature['id_service_feature'];
        if (!ValidatePermission($Conn, $loggedInAccountId, $id_service_feature)) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk melihat daftar sediaan obat", "code" => 403], "metadata" => []]);
            exit;
        }

    } catch (PDOException $e) {
        error_log('[ListMedicationForm] DB/Permission error: ' . $e->getMessage());
    ...
    $order_by   = isset($_GET['order_by']) ? trim($_GET['order_by']) : 'medicationFormId';
    ...
    // --- 14. Count total data ---
    try {
        $countSql = "SELECT COUNT(*) AS total FROM medication_form_reference m " . $where;
        ...
        error_log('[ListMedicationForm] Count error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $sql = "SELECT
                    ...
                    m.medicationFormCategory,
                    m.medicationFormGroup
                FROM medication_form_reference m
                " . $where;
        ...
        foreach ($rows as &$row) {
            $row['medicationFormId'] = (int) $row['medicationFormId'];
        }
        unset($row);
        ...
        echo json_encode([
            "response" => [
                "message" => "Daftar sediaan obat berhasil diambil",
                "code" => 200
            ],
            "metadata" => [
                "total" => $total,
        ...
        error_log('[ListMedicationForm] Query error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }

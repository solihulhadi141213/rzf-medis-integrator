<?php
    /**
     * Delete Medication Form Reference
     * Endpoint: DELETE /_API/Reference/MedicationForm/DeleteMedicationForm.php?medicationFormId={id}
     * Header: token, account_token
     *
     * Menghapus data referensi sediaan obat (medication_form_reference) secara hard delete.
     */
    ...
    $Limiter->check("delete_medication_form", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_medication_form' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus referensi sediaan obat", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeleteMedicationForm] Auth error: ' . $e->getMessage());
    ...
    try {
        $stmt = $Conn->prepare("SELECT medicationFormId FROM medication_form_reference WHERE medicationFormId = :id LIMIT 1");
        ...
        error_log('[DeleteMedicationForm] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM medication_form_reference WHERE medicationFormId = :id");
        $stmt->execute([':id' => $medicationFormId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Referensi sediaan obat berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [
        ...
        error_log('[DeleteMedicationForm] Delete error: ' . $e->getMessage());
        http_response_code(500);

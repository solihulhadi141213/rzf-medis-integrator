<?php
    ...
    if ($provinceId <= 0) {
        ...
        echo json_encode(["response" => ["message" => "provinceId tidak valid", "code" => 422], "metadata" => []]);
        exit;
    }
    ...
    try {
        ...
        $stmt = $Conn->prepare("SELECT cityId, name, bps, provinceId FROM region_city WHERE provinceId = :provinceId ORDER BY name ASC");
        $stmt->execute([':provinceId' => $provinceId]);
        $cities = $stmt->fetchAll(PDO::FETCH_ASSOC);

        http_response_code(200);
        echo json_encode([
            "response" => ["message" => "Data kabupaten/kota berhasil diambil", "code" => 200],
            "metadata" => ["provinceId" => $provinceId, "total" => count($cities)],
            "data" => $cities
        ]);
    } catch (Exception $e) {
        error_log('[City] ' . $e->getMessage());

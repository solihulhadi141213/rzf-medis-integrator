<?php
    ...
    $Limiter->check("get_village", 5, 60);
    ...
    if ($districtId <= 0) {
        ...
        echo json_encode(["response" => ["message" => "districtId tidak valid", "code" => 422], "metadata" => []]);
        exit;
    }
    ...
    try {
        ...
        $stmt = $Conn->prepare("SELECT villageId, name, oldCode, bps, districtId FROM region_village WHERE districtId = :districtId ORDER BY name ASC");
        $stmt->execute([':districtId' => $districtId]);
        $villages = $stmt->fetchAll(PDO::FETCH_ASSOC);

        http_response_code(200);
        echo json_encode([
            "response" => ["message" => "Data desa/kelurahan berhasil diambil", "code" => 200],
            "metadata" => ["districtId" => $districtId, "total" => count($villages)],
            "data" => $villages
        ]);
    } catch (Exception $e) {
        error_log('[Village] ' . $e->getMessage());

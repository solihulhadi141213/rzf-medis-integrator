<?php
    /**
     * List Episode Of Care Reference
     * Endpoint: GET /_API/Reference/EpisodeOfCare/EpisodeOfCareReference.php?limit=10&page=1&order_by=episodeOfCareReferenceId&short_by=DESC&keyword_by=&keyword=
     * Header: token, account_token
     *
     * Menampilkan daftar referensi Episode of Care dengan paginasi, sorting, dan pencarian.
     */
    ...
    $Limiter->check("list_episode_of_care_reference", 10, 60);
    ...
    try {
        ...
        // Validasi Permission (fitur list_episode_of_care_reference)
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = :feature_name LIMIT 1");
        $stmt->execute([':feature_name' => 'list_episode_of_care_reference']);
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Fitur list_episode_of_care_reference tidak ditemukan", "code" => 403], "metadata" => []]);
            exit;
        }
        $id_service_feature = (int) $feature['id_service_feature'];
        if (!ValidatePermission($Conn, $loggedInAccountId, $id_service_feature)) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk melihat daftar referensi Episode of Care", "code" => 403], "metadata" => []]);
            exit;
        }

    } catch (PDOException $e) {
        error_log('[ListEpisodeOfCareReference] DB/Permission error: ' . $e->getMessage());
    ...
    $order_by   = isset($_GET['order_by']) ? trim($_GET['order_by']) : 'episodeOfCareReferenceId';
    ...
    if (empty($order_by)) {
        $order_by = 'episodeOfCareReferenceId';
    }
    if (empty($short_by)) {
        $short_by = 'DESC';
    }
    ...
    // --- 14. Count total data ---
    try {
        $countSql = "SELECT COUNT(*) AS total FROM episode_of_care_reference e " . $where;
        ...
        error_log('[ListEpisodeOfCareReference] Count error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $sql = "SELECT
                    e.episodeOfCareReferenceId,
                    e.episodeOfCareTypeCode,
                    e.episodeOfCareTypeDisplay,
                    e.episodeOfCareTypeSystem
                FROM episode_of_care_reference e
                " . $where;
        ...
        foreach ($rows as &$row) {
            $row['episodeOfCareReferenceId'] = (int) $row['episodeOfCareReferenceId'];
        }
        unset($row);
        ...
        echo json_encode([
            "response" => [
                "message" => "Daftar referensi Episode of Care berhasil diambil",
                "code" => 200
            ],
            "metadata" => [
                "total" => $total,
        ...
        error_log('[ListEpisodeOfCareReference] Query error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }

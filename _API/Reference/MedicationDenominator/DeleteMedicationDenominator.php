<?php
    /**
     * Delete Medication Denominator Reference
     * Endpoint: DELETE /_API/Reference/MedicationDenominator/DeleteMedicationDenominator.php?medicationDenominatorId={id}
     * Header: token, account_token
     *
     * Menghapus data referensi denominator (medication_denominator) secara hard delete.
     */
    ...
    $Limiter->check("delete_medication_denominator", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_medication_denominator' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus referensi denominator", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeleteMedicationDenominator] Auth error: ' . $e->getMessage());
    ...
    try {
        $stmt = $Conn->prepare("SELECT medicationDenominatorId FROM medication_denominator WHERE medicationDenominatorId = :id LIMIT 1");
        ...
        error_log('[DeleteMedicationDenominator] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM medication_denominator WHERE medicationDenominatorId = :id");
        $stmt->execute([':id' => $medicationDenominatorId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Referensi denominator berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [
        ...
        error_log('[DeleteMedicationDenominator] Delete error: ' . $e->getMessage());
        http_response_code(500);

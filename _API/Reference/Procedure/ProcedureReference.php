<?php
/**
 * List Procedure Reference
 * Endpoint: GET /_API/Reference/Procedure/ProcedureReference.php?limit=10&page=1&order_by=procedureReferenceId&short_by=DESC&keyword_by=&keyword=
 * Header: token, account_token
 *
 * Menampilkan daftar referensi tindakan (procedure_reference) dengan paginasi, sorting, dan pencarian.
 */
...
$Limiter->check("list_procedure_reference", 10, 60);
...
try {
    ...
    // Validasi Permission (fitur list_procedure_reference)
    $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = :feature_name LIMIT 1");
    $stmt->execute([':feature_name' => 'list_procedure_reference']);
    $feature = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$feature) {
        http_response_code(403);
        echo json_encode(["response" => ["message" => "Fitur list_procedure_reference tidak ditemukan", "code" => 403], "metadata" => []]);
        exit;
    }
    $id_service_feature = (int) $feature['id_service_feature'];
    if (!ValidatePermission($Conn, $loggedInAccountId, $id_service_feature)) {
        http_response_code(403);
        echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk melihat daftar referensi tindakan", "code" => 403], "metadata" => []]);
        exit;
    }

} catch (PDOException $e) {
    error_log('[ListProcedureReference] DB/Permission error: ' . $e->getMessage());
...
$order_by   = isset($_GET['order_by']) ? trim($_GET['order_by']) : 'procedureReferenceId';
...
$allowedColumns = [
    'procedureReferenceId', 'procedureCategoryName', 'procedureCategoryCode',
    'procedureCategoryDipsplay', 'procedureCategorySystem', 'procedureName',
    'procedureCode', 'procedureDisplay', 'procedureSystem',
    'bodySiteName', 'bodySiteCode', 'bodySiteDisplay', 'bodySiteSystem',
    'icd9Code', 'icd9Description', 'status', 'creatAt', 'updateAt', 'creatBy', 'updateBy'
];
...
if ($keyword !== '' && $keyword_by !== '') {
    $where .= " AND pr.`{$keyword_by}` LIKE :keyword";
    $params[':keyword'] = "%{$keyword}%";
}

// --- 14. Count total data ---
try {
    $countSql = "SELECT COUNT(*) AS total FROM procedure_reference pr " . $where;
    ...
    error_log('[ListProcedureReference] Count error: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
    exit;
}
...
try {
    $sql = "SELECT
                pr.procedureReferenceId,
                pr.procedureCategoryName,
                pr.procedureCategoryCode,
                pr.procedureCategoryDipsplay,
                pr.procedureCategorySystem,
                pr.procedureName,
                pr.procedureCode,
                pr.procedureDisplay,
                pr.procedureSystem,
            ...
            LEFT JOIN account cAccount ON pr.creatBy = cAccount.accountId
            LEFT JOIN account uAccount ON pr.updateBy = uAccount.accountId
            " . $where;
    $sql .= " ORDER BY pr.`{$order_by}` {$short_by} LIMIT {$offset}, {$limit}";
    ...
    echo json_encode([
        "response" => [
            "message" => "Daftar referensi tindakan berhasil diambil",
            "code" => 200
        ],
        "metadata" => [
            "total" => $total,
    ...
    error_log('[ListProcedureReference] Query error: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
    exit;
}

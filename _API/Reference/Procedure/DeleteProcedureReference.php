<?php
    /**
     * Delete Procedure Reference (Hard Delete)
     * Endpoint: DELETE /_API/Reference/Procedure/DeleteProcedureReference.php?procedureReferenceId={id}
     * Header: token, account_token
     * 
     * Menghapus data referensi tindakan dari database (hard delete).
    ...
    $Limiter->check("delete_procedure_reference", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_procedure_reference' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus referensi tindakan", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeleteProcedureReference] Auth error: ' . $e->getMessage());
    ...
    try {
        $stmt = $Conn->prepare("SELECT procedureReferenceId FROM procedure_reference WHERE procedureReferenceId = :id LIMIT 1");
        ...
        error_log('[DeleteProcedureReference] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM procedure_reference WHERE procedureReferenceId = :id");
        $stmt->execute([':id' => $procedureReferenceId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Referensi tindakan berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [
        ...
        error_log('[DeleteProcedureReference] Delete error: ' . $e->getMessage());
        http_response_code(500);

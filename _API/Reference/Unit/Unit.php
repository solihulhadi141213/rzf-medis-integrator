<?php
    /**
     * List Unit Reference
     * Endpoint: GET /_API/Reference/Unit/Unit.php?limit=10&page=1&order_by=unitReferenceId&short_by=DESC&keyword_by=&keyword=
     * Header: token, account_token
     *
     * Menampilkan daftar referensi satuan (unit_reference) dengan paginasi, sorting, dan pencarian.
     */
    ...
    $Limiter->check("list_unit_reference", 10, 60);
    ...
    try {
        ...
        // Validasi Permission (fitur list_unit_reference)
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = :feature_name LIMIT 1");
        $stmt->execute([':feature_name' => 'list_unit_reference']);
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Fitur list_unit_reference tidak ditemukan", "code" => 403], "metadata" => []]);
            exit;
        }
        $id_service_feature = (int) $feature['id_service_feature'];
        if (!ValidatePermission($Conn, $loggedInAccountId, $id_service_feature)) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk melihat daftar satuan", "code" => 403], "metadata" => []]);
            exit;
        }

    } catch (PDOException $e) {
        error_log('[ListUnitReference] DB/Permission error: ' . $e->getMessage());
    ...
    $order_by   = isset($_GET['order_by']) ? trim($_GET['order_by']) : 'unitReferenceId';
    ...
    $allowedColumns = [
        'unitReferenceId', 'unitName', 'unitDisplay', 'unitCode', 'unitSystem'
    ];
    ...
    if ($keyword !== '' && $keyword_by !== '') {
        $where .= " AND u.`{$keyword_by}` LIKE :keyword";
        $params[':keyword'] = "%{$keyword}%";
    }

    // --- 14. Count total data ---
    try {
        $countSql = "SELECT COUNT(*) AS total FROM unit_reference u " . $where;
        ...
        error_log('[ListUnitReference] Count error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $sql = "SELECT
                    ...
                    u.unitCode,
                    u.unitSystem
                FROM unit_reference u
                " . $where;
        $sql .= " ORDER BY u.`{$order_by}` {$short_by} LIMIT {$offset}, {$limit}";
        ...
        foreach ($rows as &$row) {
            $row['unitReferenceId'] = (int) $row['unitReferenceId'];
        }
        unset($row);
        ...
        error_log('[ListUnitReference] Query error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }

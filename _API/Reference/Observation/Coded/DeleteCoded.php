<?php
    /**
     * Delete Observation Reference Coded
     * Endpoint: DELETE /_API/Reference/Observation/Coded/DeleteCoded.php?observationReferenceCodedId={id}
     * Header: token, account_token
     * 
     * Menghapus data coded value (observation_reference_coded) berdasarkan observationReferenceCodedId.
     * Tidak ada child table yang terpengaruh (coded adalah leaf).
     */
    ...
    $Limiter->check("delete_coded", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_coded' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus coded value", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeleteCoded] Auth error: ' . $e->getMessage());
    ...
    try {
        $stmt = $Conn->prepare("SELECT observationReferenceCodedId FROM observation_reference_coded WHERE observationReferenceCodedId = :id LIMIT 1");
        ...
        error_log('[DeleteCoded] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM observation_reference_coded WHERE observationReferenceCodedId = :id");
        $stmt->execute([':id' => $observationReferenceCodedId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Coded value berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [
        ...
        error_log('[DeleteCoded] Delete error: ' . $e->getMessage());
        http_response_code(500);

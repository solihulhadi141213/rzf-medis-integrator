<?php
    /**
     * Delete Observation Reference Range
     * Endpoint: DELETE /_API/Reference/Observation/Range/DeleteRange.php?observationResultRangeId={id}
     * Header: token, account_token
     * 
     * Menghapus data range value (observation_reference_range) berdasarkan observationResultRangeId.
     * Tidak ada child table yang terpengaruh (range adalah leaf).
     */
    ...
    $Limiter->check("delete_range", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_range' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus range value", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeleteRange] Auth error: ' . $e->getMessage());
    ...
    try {
        $stmt = $Conn->prepare("SELECT observationResultRangeId FROM observation_reference_range WHERE observationResultRangeId = :id LIMIT 1");
        ...
        error_log('[DeleteRange] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM observation_reference_range WHERE observationResultRangeId = :id");
        $stmt->execute([':id' => $observationResultRangeId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Range value berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [
        ...
        error_log('[DeleteRange] Delete error: ' . $e->getMessage());
        http_response_code(500);

<?php
    ...
    $Limiter->check("list_api_key", 5, 60);
    ...
    try {
        ...
        $stmt = $Conn->prepare("SELECT id_api_key, api_name, api_description, client_id, expired_duration, datetime_creat, datetime_update, status FROM api_key ORDER BY id_api_key DESC");
        $stmt->execute();
        ...
        echo json_encode([
            "response" => [
                "message" => "List api_key berhasil diambil",
                "code" => 200
            ],
            "metadata" => [
                "total" => count($list),

<?php
    /**
     * List Medication
     * Endpoint: GET /_API/Medication/Medication.php?limit=10&page=1&order_by=medicationId&short_by=DESC&keyword_by=&keyword=
     * Header: token, account_token
     *
     * Menampilkan daftar obat/alkes dengan paginasi, sorting, dan pencarian.
     * Menampilkan informasi obat, stok, dan pembuat/update.
     */
    ...
    $Limiter->check("list_medication", 10, 60);
    ...
    try {
        ...
        // Validasi Permission (fitur list_medication)
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = :feature_name LIMIT 1");
        $stmt->execute([':feature_name' => 'list_medication']);
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Fitur list_medication tidak ditemukan", "code" => 403], "metadata" => []]);
            exit;
        }
        $id_service_feature = (int) $feature['id_service_feature'];
        if (!ValidatePermission($Conn, $loggedInAccountId, $id_service_feature)) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk melihat daftar obat/alkes", "code" => 403], "metadata" => []]);
            exit;
        }

    } catch (PDOException $e) {
        error_log('[ListMedication] DB/Permission error: ' . $e->getMessage());
    ...
    $order_by   = isset($_GET['order_by']) ? trim($_GET['order_by']) : 'medicationId';
    ...
    if (empty($order_by)) {
        $order_by = 'medicationId';
    }
    if (empty($short_by)) {
        $short_by = 'DESC';
    }

    // --- 9. Daftar kolom yang diizinkan untuk sorting dan filter (hanya dari medication) ---
    $allowedColumns = [
        'medicationId', 'satuSehatCode', 'medicationLocalCode', 'medicationName',
        'medicationGroup', 'medicationCategory', 'kfaCode', 'kfaDisplay', 'kfaSystem',
        'medicationFormName', 'medicationFormCode', 'medicationFormDisplay', 'medicationFormSystem',
        'medicationType', 'manufacturerId', 'manufacturerName',
        'CostPrice', 'ActualStock', 'MinimumStock', 'medicationStatus',
    ...
    if ($keyword !== '' && $keyword_by !== '') {
        $where .= " AND m.`{$keyword_by}` LIKE :keyword";
        $params[':keyword'] = "%{$keyword}%";
    }

    // --- 14. Count total data ---
    try {
        $countSql = "SELECT COUNT(*) AS total FROM medication m " . $where;
        ...
        error_log('[ListMedication] Count error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    // --- 15. Query utama dengan JOIN ke account ---
    try {
        $sql = "SELECT
                    m.medicationId,
                    m.satuSehatCode,
                    m.medicationLocalCode,
                    m.medicationName,
                    m.medicationGroup,
                    m.medicationCategory,
                    m.kfaCode,
                    m.kfaDisplay,
                    m.kfaSystem,
                    m.medicationFormName,
                    m.medicationFormCode,
                    m.medicationFormDisplay,
                    m.medicationFormSystem,
                    m.medicationType,
                    m.manufacturerId,
                    m.manufacturerName,
                    m.CostPrice,
                ...
                FROM medication m
                LEFT JOIN account cAccount ON m.creatBy = cAccount.accountId
                LEFT JOIN account uAccount ON m.updateBy = uAccount.accountId
                " . $where;
        $sql .= " ORDER BY m.`{$order_by}` {$short_by} LIMIT {$offset}, {$limit}";
        ...
        echo json_encode([
            "response" => [
                "message" => "Daftar obat/alkes berhasil diambil",
                "code" => 200
            ],
            "metadata" => [
                "total" => $total,
        ...
        error_log('[ListMedication] Query error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }

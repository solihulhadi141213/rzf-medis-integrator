<?php
    /**
     * Update Medication
     * Endpoint: PUT /_API/Medication/UpdateMedication.php?medicationId={id}
     * Header: token, account_token
     * Body: JSON (lihat contoh)
     *
     * - Update data medication, termasuk child: medication_multi_form dan medication_selling_price.
     * - Jika satuSehatCode terisi, update ke SATUSEHAT (PUT).
     * - Jika satuSehatCode kosong dan syarat lengkap, create ke SATUSEHAT (POST).
     * - Perbarui updateAt dan updateBy.
     */
    ...
    $Limiter->check("update_medication", 5, 60);
    ...
    try {
        ...
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'update_medication' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk mengubah obat/alkes", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[UpdateMedication] Auth error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }

    // --- 8. Ambil data existing medication ---
    try {
        $stmt = $Conn->prepare("SELECT * FROM medication WHERE medicationId = :id LIMIT 1");
        ...
        $oldLocalCode = $existing['medicationLocalCode'];
    } catch (PDOException $e) {
        error_log('[UpdateMedication] Fetch existing error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    // --- 10. Ambil nilai dari body (opsional, gunakan nilai lama jika tidak ada) ---
    // Field yang wajib: medicationName, medicationGroup, medicationCategory, medicationFormName, medicationType, medicationStatus
    $satuSehatCode      = isset($input['satuSehatCode']) ? trim($input['satuSehatCode']) : $existing['satuSehatCode'];
    $medicationLocalCode = isset($input['medicationLocalCode']) ? trim($input['medicationLocalCode']) : $existing['medicationLocalCode'];
    $medicationName     = isset($input['medicationName']) ? trim($input['medicationName']) : $existing['medicationName'];
    $medicationGroup    = isset($input['medicationGroup']) ? trim($input['medicationGroup']) : $existing['medicationGroup'];
    $medicationCategory = isset($input['medicationCategory']) ? trim($input['medicationCategory']) : $existing['medicationCategory'];
    $kfaCode            = isset($input['kfaCode']) ? trim($input['kfaCode']) : $existing['kfaCode'];
    $kfaDisplay         = isset($input['kfaDisplay']) ? trim($input['kfaDisplay']) : $existing['kfaDisplay'];
    $kfaSystem          = isset($input['kfaSystem']) ? trim($input['kfaSystem']) : $existing['kfaSystem'];
    $medicationFormName = isset($input['medicationFormName']) ? trim($input['medicationFormName']) : $existing['medicationFormName'];
    $medicationFormCode = isset($input['medicationFormCode']) ? trim($input['medicationFormCode']) : $existing['medicationFormCode'];
    $medicationFormDisplay = isset($input['medicationFormDisplay']) ? trim($input['medicationFormDisplay']) : $existing['medicationFormDisplay'];
    $medicationFormSystem  = isset($input['medicationFormSystem']) ? trim($input['medicationFormSystem']) : $existing['medicationFormSystem'];
    $medicationType     = isset($input['medicationType']) ? trim($input['medicationType']) : $existing['medicationType'];
    $manufacturerId     = isset($input['manufacturerId']) ? trim($input['manufacturerId']) : $existing['manufacturerId'];
    $manufacturerName   = isset($input['manufacturerName']) ? trim($input['manufacturerName']) : $existing['manufacturerName'];
    $medicationIngredient = isset($input['medicationIngredient']) ? $input['medicationIngredient'] : (json_decode($existing['medicationIngredient'], true) ?: []);
    $CostPrice          = isset($input['CostPrice']) && $input['CostPrice'] !== '' ? (float) $input['CostPrice'] : (float) $existing['CostPrice'];
    $ActualStock        = isset($input['ActualStock']) && $input['ActualStock'] !== '' ? (float) $input['ActualStock'] : (float) $existing['ActualStock'];
    $MinimumStock       = isset($input['MinimumStock']) && $input['MinimumStock'] !== '' ? (float) $input['MinimumStock'] : (float) $existing['MinimumStock'];
    $medicationStatus   = isset($input['medicationStatus']) ? trim($input['medicationStatus']) : $existing['medicationStatus'];
    ...
    // --- 12. Validasi medicationLocalCode (jika berubah, cek duplikat) ---
    if ($medicationLocalCode !== $oldLocalCode) {
        ...
        try {
            $stmt = $Conn->prepare("SELECT medicationId FROM medication WHERE medicationLocalCode = :code AND medicationId != :id LIMIT 1");
            $stmt->execute([':code' => $medicationLocalCode, ':id' => $medicationId]);
            if ($stmt->fetch()) {
                http_response_code(409);
                echo json_encode(["response" => ["message" => "medicationLocalCode sudah digunakan oleh data lain", "code" => 409], "metadata" => []]);
                exit;
            }
        } catch (PDOException $e) {
            error_log('[UpdateMedication] Check code error: ' . $e->getMessage());
            http_response_code(500);
            echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
            exit;
        }
    ...
    // --- 16. Mulai transaksi untuk update master dan child ---
    $Conn->beginTransaction();
    try {
        // 16a. Update medication
        ...
        // 16b. Hapus child lama
        $Conn->prepare("DELETE FROM medication_multi_form WHERE medicationId = :id")->execute([':id' => $medicationId]);
        $Conn->prepare("DELETE FROM medication_selling_price WHERE medicationId = :id")->execute([':id' => $medicationId]);

        // 16c. Insert multi_form baru
        if (!empty($multiForm)) {
            ...
            foreach ($multiForm as $item) {
                $stmt->execute([
                    ...
                    ':medicationFormCode' => $item['medicationFormCode'] ?? null,
                    ':medicationFormDisplay' => $item['medicationFormDisplay'] ?? null,
                    ':medicationFormSystem' => $item['medicationFormSystem'] ?? null,
                    ':conversionFactor' => (float) $item['conversionFactor']
                ]);
            }
        }

        // 16d. Insert selling_price baru
        if (!empty($sellingPrices)) {
            ...
            foreach ($sellingPrices as $item) {
                $stmt->execute([
                    ...
                    ':medicationSellingPriceDescription' => $item['medicationSellingPriceDescription'] ?? null,
        ...
        $Conn->rollBack();
        error_log('[UpdateMedication] Update transaction error: ' . $e->getMessage());
        http_response_code(500);
    ...
    $satusehatCode = $satuSehatCode; // default
    ...
    try {
        ...
        error_log('[UpdateMedication] Get credential error: ' . $e->getMessage());
    }

    // Syarat sync: semua field penting terisi
    ...
    if ($canSync) {
        try {
            ...
            if ($tokenResult['status'] === 'success') {
                ...
                // Siapkan payload (sama seperti create)
                $payload = [
                    ...
                    'status' => ($medicationStatus === 'Available' || $medicationStatus === 'Registered') ? 'active' : 'inactive'
                ];
                ...
                // Tentukan metode: jika satuSehatCode ada (tidak kosong) -> PUT, else POST
                $method = !empty($satuSehatCode) ? 'PUT' : 'POST';
                $url = $baseUrl . '/fhir-r4/v1/' . $resourceType;
                if ($method === 'PUT') {
                    $url .= '/' . $satuSehatCode;
                    $payload['id'] = $satuSehatCode;
                }
                ...
                if ($response === false) {
                    ...
                    if (isset($result['id'])) {
                        ...
                        // Jika POST (create) dan sebelumnya tidak ada satuSehatCode, update di lokal
                        if ($method === 'POST') {
                        ...
                        $satusehatMessage = 'Berhasil sinkronisasi ke SATUSEHAT';
                    } else {
            ...
            error_log('[UpdateMedication] SATUSEHAT integration error: ' . $e->getMessage());
            $satusehatSyncStatus = 'failed';
        }
    } else {
    ...
    // --- 18. Ambil data yang baru diperbarui untuk response ---
    $row = null;
    try {
        $stmt = $Conn->prepare("
            SELECT m.*,
        ...
        error_log('[UpdateMedication] Fetch updated data error: ' . $e->getMessage());
        $row = null;
    }
    ...
    echo json_encode([
        "response" => [
            "message" => "Obat/Alkes berhasil diperbarui",
            "code" => 200
        ],
        "metadata" => [
            "medicationId" => $medicationId,

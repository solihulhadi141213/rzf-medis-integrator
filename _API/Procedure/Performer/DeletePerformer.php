<?php
     ...
     * Endpoint: DELETE /_API/Procedure/Performer/DeletePerformer.php?procedurePerformerId={id}
     * Header: token, account_token
     * 
     * Menghapus data performer dari prosedur (hard delete).
     */
    ...
    $Limiter->check("delete_procedure_performer", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_procedure_performer' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus performer tindakan", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeletePerformer] Auth error: ' . $e->getMessage());
    ...
    try {
        $stmt = $Conn->prepare("SELECT procedurePerformerId FROM procedure_performer WHERE procedurePerformerId = :id LIMIT 1");
        ...
        error_log('[DeletePerformer] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM procedure_performer WHERE procedurePerformerId = :id");
        $stmt->execute([':id' => $procedurePerformerId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Performer berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [
        ...
        error_log('[DeletePerformer] Delete error: ' . $e->getMessage());
        http_response_code(500);

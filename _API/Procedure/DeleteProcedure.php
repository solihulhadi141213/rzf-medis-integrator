<?php
     ...
     * Endpoint: DELETE /_API/Procedure/DeleteProcedure.php?procedureId={id}
     * Header: token, account_token
     * 
     * Menghapus data tindakan (procedure_encounter) dari database (hard delete).
     * Data procedure_performer terhapus otomatis karena ON DELETE CASCADE.
    ...
    $Limiter->check("delete_procedure", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_procedure' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus tindakan", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeleteProcedure] Auth error: ' . $e->getMessage());
    ...
    try {
        ...
        error_log('[DeleteProcedure] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    // Procedure_performer akan terhapus otomatis karena ON DELETE CASCADE
    try {
        $stmt = $Conn->prepare("DELETE FROM procedure_encounter WHERE procedureId = :id");
        $stmt->execute([':id' => $procedureId]);
        ...
        error_log('[DeleteProcedure] Delete error: ' . $e->getMessage());
        http_response_code(500);

<?php
    /**
     * Delete Allergy (Hard Delete)
     * Endpoint: DELETE /_API/Allergy/DeleteAllergy.php?allergyId={id}
     * Header: token, account_token
     * 
     * Menghapus data alergi pasien (hard delete) berdasarkan allergyId.
    ...
    $Limiter->check("delete_allergy", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_allergy' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus alergi pasien", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeleteAllergy] Auth error: ' . $e->getMessage());
    ...
    try {
        $stmt = $Conn->prepare("SELECT allergyId FROM allergy WHERE allergyId = :id LIMIT 1");
        ...
        error_log('[DeleteAllergy] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM allergy WHERE allergyId = :id");
        $stmt->execute([':id' => $allergyId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Data alergi pasien berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [
                "allergyId" => $allergyId,
        ...
        error_log('[DeleteAllergy] Delete error: ' . $e->getMessage());
        http_response_code(500);

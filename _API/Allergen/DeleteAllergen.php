<?php
    /**
     * Delete Allergen
     * Endpoint: DELETE /_API/Allergen/DeleteAllergen.php?AllergenId={id}
     * Header: token, account_token
     *
     * Menghapus data alergen (hard delete) berdasarkan AllergenId.
     */
    ...
    $Limiter->check("delete_allergen", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_allergen' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus alergen", "code" => 403], "metadata" => []]);
            exit;
        }
    } catch (PDOException $e) {
        error_log('[DeleteAllergen] Auth error: ' . $e->getMessage());
    ...
    try {
        $stmt = $Conn->prepare("SELECT AllergenId FROM allergen WHERE AllergenId = :id LIMIT 1");
        ...
        error_log('[DeleteAllergen] Check data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM allergen WHERE AllergenId = :id");
        $stmt->execute([':id' => $allergenId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Alergen berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [
        ...
        error_log('[DeleteAllergen] Delete error: ' . $e->getMessage());
        http_response_code(500);

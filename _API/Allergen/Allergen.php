<?php
    /**
     * List Allergen Reference
     * Endpoint: GET /_API/Allergen/Allergen.php?limit=10&page=1&order_by=AllergenId&short_by=DESC&keyword_by=&keyword=
     * Header: token, account_token
     * 
     * Menampilkan daftar referensi alergen dengan paginasi, sorting, dan pencarian.
     */
    ...
    $Limiter->check("list_allergen", 10, 60);
    ...
    try {
        ...
        // Validasi Permission (fitur list_allergen)
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = :feature_name LIMIT 1");
        $stmt->execute([':feature_name' => 'list_allergen']);
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Fitur list_allergen tidak ditemukan", "code" => 403], "metadata" => []]);
            exit;
        }
        $id_service_feature = (int) $feature['id_service_feature'];
        if (!ValidatePermission($Conn, $loggedInAccountId, $id_service_feature)) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk melihat daftar alergen", "code" => 403], "metadata" => []]);
            exit;
        }

    } catch (PDOException $e) {
        error_log('[ListAllergen] DB/Permission error: ' . $e->getMessage());
    ...
    $order_by   = isset($_GET['order_by']) ? trim($_GET['order_by']) : 'AllergenId';
    ...
    $allowedColumns = [
        'AllergenId', 'allergenCategory', 'allergenName', 'allergenCode',
        'allergenDisplay', 'allergenSystem', 'creatAt', 'updateAt', 'creatBy', 'updateBy'
    ];
    ...
    // --- 14. Count total data ---
    try {
        $countSql = "SELECT COUNT(*) AS total FROM allergen a " . $where;
        ...
        error_log('[ListAllergen] Count error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }
    ...
    try {
        ...
        foreach ($rows as &$row) {
            $row['AllergenId'] = (int) $row['AllergenId'];
        ...
        error_log('[ListAllergen] Query error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(["response" => ["message" => "Internal Server Error", "code" => 500], "metadata" => []]);
        exit;
    }

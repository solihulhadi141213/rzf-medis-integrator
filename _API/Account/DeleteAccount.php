<?php
    ...
    $Limiter->check("delete_account", 5, 60);
    ...
    $accountIdToDelete = isset($_GET['accountId']) ? (int) $_GET['accountId'] : 0;
    ...
    try {
        ...
        $stmt->execute([':feature_name' => 'delete_account']);
        $feature = $stmt->fetch();
        if (!$feature) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Fitur delete_account tidak ditemukan", "code" => 403], "metadata" => []]);
            exit;
        }
        ...
        if (!ValidatePermission($Conn, $accountId, $id_service_feature)) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus akun", "code" => 403], "metadata" => []]);
            exit;
        }

        $stmt = $Conn->prepare("SELECT accountId, photo FROM account WHERE accountId = :accountId LIMIT 1");
        $stmt->execute([':accountId' => $accountIdToDelete]);
        ...
        if (!empty($accountData['photo'])) {
            $photoPath = __DIR__ . '/../../Storage/Img/Account/' . $accountData['photo'];
            if (file_exists($photoPath) && is_file($photoPath)) {
                @unlink($photoPath);
            }
        }

        $stmt = $Conn->prepare("DELETE FROM account WHERE accountId = :accountId");
        $stmt->execute([':accountId' => $accountIdToDelete]);

        http_response_code(200);
        echo json_encode([
            "response" => ["message" => "Account berhasil dihapus", "code" => 200],
            "metadata" => ["accountId" => $accountIdToDelete]
        ]);
    } catch (Exception $e) {

<?php
     ...
     * Endpoint: POST /_API/Inpatient/Room/CreateInpatientRoom.php
     * Header: token, account_token
     * Body: JSON { "inpatientClassId": 9, "inpatientRoomCode": "RM-1", "inpatientRoomName": "Ruang Mawar" }
     * 
     * Sistem akan membuat Location di SATUSEHAT dengan partOf mengacu ke Location kelas rawat inap.
     */
    ...
    $Limiter->check("create_inpatient_room", 5, 60);
    ...
    try {
        ...
        // Validasi Permission (fitur create_inpatient_room)
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = :feature_name LIMIT 1");
        $stmt->execute([':feature_name' => 'create_inpatient_room']);
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature) {
            http_response_code(403);
            echo json_encode([
                "response" => [
                    "message" => "Fitur create_inpatient_room tidak ditemukan",
                    "code" => 403
                ],
                "metadata" => []
            ]);
        ...
        if (!ValidatePermission($Conn, $loggedInAccountId, $id_service_feature)) {
            ...
            echo json_encode([
                "response" => [
                    "message" => "Tidak memiliki izin untuk menambah ruangan rawat inap",
                    "code" => 403
                ],
                "metadata" => []
            ]);
        ...
        error_log('[CreateInpatientRoom] DB/Permission error: ' . $e->getMessage());
    ...
    $requiredFields = ['inpatientClassId', 'inpatientRoomCode', 'inpatientRoomName'];
    ...
    $inpatientClassId = (int) $input['inpatientClassId'];
    $inpatientRoomCode = trim($input['inpatientRoomCode']);
    $inpatientRoomName = trim($input['inpatientRoomName']);
    $status = isset($input['status']) ? (int) $input['status'] : 1;

    // --- 10. Validasi inpatientClassId (cek keberadaan) ---
    try {
        ...
        $classSatuSehatCode = $classData['satuSehatCode'];
    } catch (PDOException $e) {
        error_log('[CreateInpatientRoom] Check class error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode([
            "response" => [
                "message" => "Internal Server Error",
    ...
    // --- 11. Validasi inpatientRoomCode (unik, max 20) ---
    ...
    try {
        $stmt = $Conn->prepare("SELECT inpatientRoomId FROM inpatient_room WHERE inpatientRoomCode = :code LIMIT 1");
        $stmt->execute([':code' => $inpatientRoomCode]);
        if ($stmt->fetch()) {
            http_response_code(409);
            echo json_encode([
                "response" => [
                    "message" => "inpatientRoomCode sudah digunakan",
                    "code" => 409
                ],
                "metadata" => []
            ]);
        ...
        error_log('[CreateInpatientRoom] Check code error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode([
            "response" => [
                "message" => "Internal Server Error",
    ...
    // --- 13. Kirim ke SATUSEHAT untuk membuat Location ---
    try {
        ...
        // Siapkan data untuk Location
        $locationData = [
            "resourceType" => "Location",
            "identifier" => [
                [
                    "system" => "http://sys-ids.kemkes.go.id/location/$organizationId",
                    "value" => $inpatientRoomCode
                ]
            ],
            "name" => $inpatientRoomName,
            "status" => "active",
            "partOf" => [
                "reference" => "Location/" . $classSatuSehatCode
            ]
        ];
        ...
        error_log('[CreateInpatientRoom] SATUSEHAT error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode([
            "response" => [
    ...
    try {
        ...
        $sql = "INSERT INTO inpatient_room (
                    inpatientClassId,
                    inpatientRoomCode,
                    satuSehatCode,
                    inpatientRoomName,
        ...
        echo json_encode([
            "response" => [
                "message" => "Ruangan rawat inap berhasil ditambahkan dan disinkronkan ke SATUSEHAT",
                "code" => 201
            ],
            "metadata" => [
                "inpatientRoomId" => $newId,
                "satuSehatCode" => $satuSehatCode,
        ...
        error_log('[CreateInpatientRoom] Insert error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode([
            "response" => [

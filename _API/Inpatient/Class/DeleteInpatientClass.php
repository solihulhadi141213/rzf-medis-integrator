<?php
    /**
     * Delete Inpatient Class (Hard Delete)
     * Endpoint: DELETE /_API/Inpatient/Class/DeleteInpatientClass.php?inpatientClassId={id}
     * Header: token, account_token
     * 
     * Jika satuSehatCode terisi, akan melakukan update status menjadi "inactive" di SATUSEHAT sebelum menghapus data lokal.
     * Data terkait (inpatient_room dan inpatient_bed) akan terhapus otomatis karena ON DELETE CASCADE.
     */
    ...
    $Limiter->check("delete_inpatient_class", 5, 60);
    ...
    try {
        ...
        // Validasi Permission (fitur delete_inpatient_class)
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = :feature_name LIMIT 1");
        $stmt->execute([':feature_name' => 'delete_inpatient_class']);
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature) {
            http_response_code(403);
            echo json_encode([
                "response" => [
                    "message" => "Fitur delete_inpatient_class tidak ditemukan",
                    "code" => 403
                ],
                "metadata" => []
            ]);
        ...
        if (!ValidatePermission($Conn, $loggedInAccountId, $id_service_feature)) {
            ...
            echo json_encode([
                "response" => [
                    "message" => "Tidak memiliki izin untuk menghapus kelas rawat inap",
                    "code" => 403
                ],
                "metadata" => []
            ]);
        ...
        error_log('[DeleteInpatientClass] DB/Permission error: ' . $e->getMessage());
    ...
    // --- 8. Ambil data kelas rawat inap yang akan dihapus ---
    try {
        ...
        $inpatientClassName = $existingData['inpatientClassName'];
    } catch (PDOException $e) {
        error_log('[DeleteInpatientClass] Fetch data error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode([
            "response" => [
                "message" => "Internal Server Error",
    ...
    if (!empty($satuSehatCode)) {
        try {
            ...
            error_log('[DeleteInpatientClass] SATUSEHAT error: ' . $e->getMessage());
            http_response_code(500);
            echo json_encode([
                "response" => [
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM inpatient_class WHERE inpatientClassId = :id");
        $stmt->execute([':id' => $inpatientClassId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Kelas rawat inap berhasil dihapus" . (!empty($satuSehatCode) ? " dan status SATUSEHAT diubah menjadi inactive" : ""),
                "code" => 200
            ],
            "metadata" => [
        ...
        error_log('[DeleteInpatientClass] Delete error: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode([
            "response" => [

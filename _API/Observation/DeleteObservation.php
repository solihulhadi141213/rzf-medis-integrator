<?php
     ...
     * Endpoint: DELETE /_API/Observation/DeleteObservation.php?observationResumeId={id}
     * Header: token, account_token
     *
     * Menghapus data hasil observasi (observation_result) secara hard delete.
     * Tidak ada sinkronisasi ke SATUSEHAT, hanya hapus lokal.
     * Perhatikan: Ada foreign key ke tabel lain, pastikan tidak ada child records yang terikat.
     */
    ...
    $Limiter->check("delete_observation", 5, 60);
    ...
    try {
        ...
        // Permission
        $stmt = $Conn->prepare("SELECT id_service_feature FROM service_feature WHERE feature_name = 'delete_observation' LIMIT 1");
        $stmt->execute();
        $feature = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$feature || !ValidatePermission($Conn, $loggedInAccountId, $feature['id_service_feature'])) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk menghapus hasil observasi", "code" => 403], "metadata" => []]);
            exit;
        }
    ...
    try {
        $stmt = $Conn->prepare("SELECT observationResumeId FROM observation_result WHERE observationResumeId = :id LIMIT 1");
    ...
    try {
        $stmt = $Conn->prepare("DELETE FROM observation_result WHERE observationResumeId = :id");
        $stmt->execute([':id' => $observationResumeId]);
        ...
        echo json_encode([
            "response" => [
                "message" => "Hasil observasi berhasil dihapus",
                "code" => 200
            ],
            "metadata" => [

<?php
    ...
    $Limiter->check("credential_status", 5, 60);
    ...
    try {
        ...
        $stmt->execute([':feature_name' => 'credential_status']);
        $feature = $stmt->fetch();
        if (!$feature) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Fitur credential_status tidak ditemukan", "code" => 403], "metadata" => []]);
            exit;
        }
        ...
        if (!ValidatePermission($Conn, $accountId, $id_service_feature)) {
            http_response_code(403);
            echo json_encode(["response" => ["message" => "Tidak memiliki izin untuk melihat status SATUSEHAT", "code" => 403], "metadata" => []]);
            exit;
        }

        $credentialStmt = $Conn->prepare("SELECT credentialId, credentialName, baseUrl, organizationId, clientKey, secretKey, status, token, tokenExpired, createdDate, updatedDate FROM satusehat WHERE status = 1 LIMIT 1");
        $credentialStmt->execute();
        $credentialData = $credentialStmt->fetch(PDO::FETCH_ASSOC);

        $connectionResult = generateTokenSatusehat($Conn);
        ...
        if ($connectionResult['status'] === 'success') {
            ...
            echo json_encode([
                "response" => [
                    "message" => "Status koneksi SATUSEHAT berhasil diperoleh",
                    "code" => 200
                ],
                "metadata" => [
            ...
            echo json_encode([
                "response" => [
                    "message" => "Gagal menguji koneksi SATUSEHAT",
                    "code" => 502
                ],
                "metadata" => [
                    "credential" => $credentialData,
        ...
        error_log('[CredentialStatus] ' . $e->getMessage());
